...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = (
        ...
        "• **记录收入:** 输入 `+金额` (例: `+1000`)\n"
        "• **记录支出:** 输入 `-金额` (例: `-500`)\n"
        "• **查看账单:** 输入 `/show` 或 `/账单` 查看当前汇总\n"
        "• **撤销错误:** 输入 `/undo` 或 `/撤回` 删除最后一条记录\n\n"
        
        "👑 **2. 组长管理指令 (需要权限)**\n"
        "组长可以通过 **回复(Reply)** 成员的消息来管理：\n"
        "• **授权成员:** 回复成员消息 + `/add` 或 `/授权` \n"
        "• **取消授权:** 回复成员消息 + `/remove` 或 `/取消` \n"
        "• **清空账目:** 输入 `/reset` 或 `/重置` (慎用！将清空全群记录)\n\n"
        
        "💳 **3. 权限激活与续费 (私聊使用)**\n"
        "• **开通权限:** 在私聊中发送 `/start` 获取转账地址\n"
        "• **自动激活:** 转账成功后，系统将在 1 分钟内通过区块链验证并自动激活，**无需发送账单截图**。\n"
        "• **查询状态:** 发送 `/check` 或 `/查询` 查看剩余天数\n\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
        "⚠️ **注意事项:**\n"
        "1. 转账时请务必按照系统生成的 **精准金额**(包含小数部分)进行转账。\n"
        "2. 权限到期后，机器人将停止记账功能，请及时续费。"
    )
    await update.message.reply_text(msg, parse_mode='Markdown')
