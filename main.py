...
async def auto_verify_task(context: ContextTypes.DEFAULT_TYPE):
    """ระบบตรวจสอบยอดเงินอัตโนมัติในพื้นหลัง"""
    try:
        conn = get_db_connection(); cursor = conn.cursor()
                ...
                for tx in data.get('data', []):
                    if abs((int(tx['value'])/1000000) - float(amt)) < 0.0001:
                        ...
                        if not cursor.fetchone():
                            ...
                            conn.commit()
                            await context.bot.send_message(chat_id=uid, text=f"✅ **支付成功！** 您的权限已开通至: `{new_exp.strftime('%Y-%m-%d %H:%M')}`", parse_mode='Markdown')
